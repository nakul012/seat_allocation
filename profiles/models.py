...
class Student(Timestampedmodel):
    ...
    def get_seat(self, date):
        seat_history = SeatHistory.objects.filter(Q(student_id=self.id, filled_at__lte=date, vacant_at__isnull=True) | Q(student_id=self.id, filled_at__lte=date, vacant_at__gte=date))
        #Q(SeatHistory.objects.filter(student_id=self.id, filled_at__lte=date)) | Q(SeatHistory.objects.filter(student_id=self.id, filled_at__lte=date, vacant_at__gte=date))
        #if seat_history:
        seat_history=seat_history.objects.filter(datetime_published__year='2022', 
                         datetime_published__month='02', 
                         datetime_published__day='24')
        return seat_history
        #else:
            #raise ValidationError(f'Given student is not seated at given {date}')
...
#(value list and flat)https://stackoverflow.com/questions/37205793/django-values-list-vs-values
#The values_list() method returns a QuerySet containing tuples and flat will remove the tuple.
